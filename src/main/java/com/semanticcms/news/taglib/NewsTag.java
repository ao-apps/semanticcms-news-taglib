...
public class NewsTag extends ElementTag<News> {
  ...
  private ValueExpression domainExpr;
  ...
  protected void evaluateAttributes(News news, ELContext elContext) throws JspTagException {
    try {
      ...
      news.setDomain(
          DomainName.valueOf(
              nullIfEmpty(
                  resolveValue(domainExpr, String.class, elContext)
              )
          )
      );
      news.setBook(
          Path.valueOf(
              nullIfEmpty(
                  resolveValue(bookExpr, String.class, elContext)
              )
          )
      );
      ...
      if (viewStr == null) {
        viewStr = Link.DEFAULT_VIEW_NAME;
      }
  ...
  protected void doBody(News news, CaptureLevel captureLevel) throws JspException, IOException {
    ...
    try {
      NewsHtmlRenderer.doBodyImpl(servletContext, request, response, news);
